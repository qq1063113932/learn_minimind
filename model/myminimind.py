...
def precompute_freqs(
    ...
    if rope_scaling is not None:
        ...
        if end / original_max > 1.0: 
            corr_dim = next(i for i in range(dim // 2) if 2 * math.pi / freqs[i] > original_max)

            power = torch.arange(0, dim // 2, device=freqs.device).float() / max(1, dim // 2 -1)

            beta = beta_slow + power * (beta_fast - beta_slow) 
...
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    x = x[:, :, :, None, :].expand(bs, slen, num_key_value_heads, n_rep, head_dim).reshape(bs, slen, num_key_value_heads * n_rep, head_dim)
    return x
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        xk = xk.view(bs, slen, self.num_key_value_heads, self.head_dim)
        xv = xv.view(bs, slen, self.num_key_value_heads, self.head_dim)

        cos, sin = position_embeddings

        xq, xk = apply_rotary_pos_emb(xq, xk, cos, sin, position_ids=None) #RoPE
        ...
        if (
            ...
            attn_mask = (
                ...
                else attention_mask.view(bs, 1, 1, -1)
                .expand(bs, self.n_local_heads, slen, -1)
                .bool()
            )
...
class MyMindModel(nn.Module):
    def __init__(self, config: MokioMindConfig):
        ...
        self.layers = nn.ModuleList(
            [
                MyMindBlock(i, config)
                for i in range(self.num_hidden_layers)
            ]
        )
        self.final_ln = RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
